...
// Paragraph Word Colors
const colorCurrentWord = "#dddddd"; // Dark Gray
const colorCorrectWord = "#93C572"; // Green
const colorIncorrectWord = "#e50000"; // Red

// Function to add words in paragraph
function addWords() 
{
	...
	for (let i = 350; i > 0; i--) 
	{
		...
		wordSection.innerHTML = wordSection.innerHTML + wordSpan;
	}
...
// Function to shuffle words from Array to Paragraph
function shuffle(array) 
{
	let len = array.length, temp, i;
...
function typingTest(e) 
{
	...
	wordData.typed = wordData.typed + 1;
}

// Function to check if time limit has exceeded
...
{   
	...
	if (checkWord(word)) 
	{
		...
		wordData.correct = wordData.correct + 1;
	} 
	else 
	{
		current.classList.remove("current-word", "incorrect-word-bg");
		current.classList.add("incorrect-word-c");
		wordData.incorrect = wordData.incorrect + 1;
	}
...
function calculateWPM(data) 
{
	...
	<ul id="results">
		<li> 
			WPM: <span class="wpm-value">${wpm}</span>
		</li>
		<li> 
			Accuracy: <span class="wpm-value">${accuracy}%</span>
		</li>
		<li id="results-stats">
